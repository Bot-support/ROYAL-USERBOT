...
from ROYAL import CMD_HELP
from ROYAL.helpers.functions import trumptweet, changemymind, kannagen, moditweet, miatweet, papputweet, sunnytweet, sinstweet, taklatweet, deEmojify, tweets
from ROYAL.utils import admin_cmd, edit_or_reply, sudo_cmd

@bot.on(admin_cmd(pattern=r"tweet(?: |$)(.*)"))
@bot.on(sudo_cmd(pattern="tweet(?: |$)(.*)", allow_sudo=True))
async def nope(patriciaX22):
    hell = patriciaX22.pattern_match.group(1)
    if not hell:
        if patriciaX22.is_reply:
            what = (await patriciaX22.get_reply_message()).message
        else:
            await patriciaX22.edit("I need some text to make a tweet🚶")
            return
    tweeter = await bot.inline_query(
        "TwitterStatusBot", f"{(deEmojify(hell))}")
    await tweeter[0].click(legendx22.chat_id,
                            reply_to=patriciaX22.reply_to_msg_id,
                            silent=True if patriciaX22.is_reply else False,
                            hide_via=True)
    await patriciaX22.delete()

...
from ROYAL import CMD_LIST, CMD_HELP
import io
...
from ROYAL.utils import remove_plugin,load_module
#Making The Back Command Was The Toughest Work #by @Shivam_Patel,@The_Siddharth_Nigam,@danish_00,@ProgrammingError also v changed Pop up or inline help to text
#A legend bhai chori karna aaya ho kya friday me ek bar back btn kang kar k man nahi bhara 
#Agar legend nahi ho to kon hai be tu jo bhi hai kang karna he aaya hai mera back , open btn so get lost
...
# 🇨 🇭 🇦 🇱 🇴      🇸 🇮 🇷    🇵 🇱 🇪 🇦 🇸 🇪    🇬 🇪 🇹 🇴 🇺 🇹    
    ...
    async def opner(event):
            if event.query.user_id == bot.uid or event.query.user_id == id:
                ...
                reply_pop_up_alert = "Please get your own Userbot,for more info visit @ROYAL_USERBOT_SUPPORT !"
  ...
  #       🇮 🇹 🇳 🇦    🇰 🇾 🇺   🇸 🇵 🇾     🇰 🇷    🇷 🇭 🇪     🇭 🇴      🇸 🇭 🇦 🇦 🇩 🇮    🇰 🇷 🇳 🇮    🇭    🇰 🇾 🇦   🇧 🇸 🇩 🇰 
    ...
    async def inline_handler(event):
        ...
        if event.query.user_id == bot.uid and query.startswith("Userbot") or event.query.user_id == id and query.startswith("Userbot"):
            ...
            result = builder.article("© ROYAL BOT Userbot Help",text="{}\nCurrently Loaded Plugins: {}".format(query, len(CMD_LIST)),buttons=dc,link_preview=False)
            await event.answer([result] if result else None)
        else:
              reply_pop_up_alert = "Please get your own Royalbot free h 😜,for more info visit @ROYAL_USERBOT_SUPPORT! 😎😎"
              await event.answer(reply_pop_up_alert, cache_time=0, alert=True)
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid or event.query.user_id == id:  # pylint:disable=E0602
            ...
            Cobra = "Please get your own Userbot, and don't use mine for more info visit @ROYAL_USERBOT_SUPPORT!"
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid or event.query.user_id == id:  # pylint:disable=E0602
            ...
            await event.edit(buttons=dc)
              ...
              TheROYAL = "Please get your own ROYALBOT FREE H 😜,for more info visit @ROYAL_USERBOT_SUPPORT! 😎😎"
              await event.answer(TheROYAL, cache_time=0, alert=True)
 #hehehehehhehhehhehe   
    ...
    async def on_plug_in_callback_query_handler(event):
        if not event.query.user_id == bot.uid or event.query.user_id == id:
            atul= "Please get your own ROYALBOT FREE H 😜,for more info visit @ROYAL_USERBOT_SUPPORT! 😎😎"
        ...
        try:
            if event.query.user_id == bot.uid or event.query.user_id == id:
                ...
                reply_pop_up_alert = "Please get your own Userbot, and don't use mine for more info visit @patriciaX!"
...
#  🇨  🇦 ‌ ‌🇳‌     
                    ...
                    try:
                        ...
                        load_module(event.data_match.group(1).decode("UTF-8"))#idhr ane ka nhi wrna g mar li jayegi
                        await event.edit( "`Your ROYAL BOT Has Successfully loaded` >>>" + str(event.data_match.group(1).decode("UTF-8")),buttons=fcix)
                    except Exception as e:
                        await event.edit("Error{}".format(shortname, str(e))+ "ROYAL BOT Has Successfully loaded" + str(event.data_match.group(1).decode("UTF-8")),buttons=fcix)
              else:
                    shortname = event.data_match.group(1).decode("UTF-8")
                    fcix = [custom.Button.inline("  𝕭𝖆𝖈𝖐 ",data="back({})".format(shivam)),custom.Button.inline(" 𝕮𝖑𝖔𝖘𝖊 ", data="close"),custom.Button.inline(" 𝖀𝖓𝖑𝖔𝖆𝖉 ",data="unload({})".format(shivam_sh1vam))]
                    reply_pop_up_alert = "Please get your own Userbot,for more info visit @patriciaX!"
                    await event.answer(reply_pop_up_alert, cache_time=0, alert=True)
    ...
    async def on_plug_in_callback_query_handler(event):
              if event.query.user_id == bot.uid or event.query.user_id == id:
                    ...
                    try:
                        ...
                        remove_plugin(event.data_match.group(1).decode("UTF-8"))#idhr ane ka nhi wrna g mar li jayegi
                        await event.edit( "`Your ROYAL BOT Has Successfully unloaded` >>>" + str(event.data_match.group(1).decode("UTF-8")),buttons=fcix)
                    except Exception as e:
                        await event.edit("Error{}".format(shortname, str(e)) +"ROYALBOT Has Successfully unloaded"+ str(event.data_match.group(1).decode("UTF-8")),buttons=fcix)
              else:
                    shortname = event.data_match.group(1).decode("UTF-8")
                    fcix = [custom.Button.inline("  𝕭𝖆𝖈𝖐 ",data="back({})".format(shivam)),custom.Button.inline(" 𝕮𝖑𝖔𝖘𝖊 ", data="close"),custom.Button.inline(" 𝕷𝖔𝖆𝖉 ",data="load({})".format(shivam_sh1vam))]
                    reply_pop_up_alert = "Please get your own Userbot,for more info visit @ROYAL_USERBOT_SUPPORT!"
    ...
    async def on_plug_in_callback_query_handler(event):
            ...
            if event.query.user_id == bot.uid or event.query.user_id == id:
                try:
                    ...
                    await event.edit("`>>> Here Is The Main Menu of\n\n©ROYALBOT`", buttons=buttons)
                except:
                    buttons = paginate_help(0, CMD_HELP, "helpme")
                    await event.edit("`>>> Here Is The Main Menu Of\n\n©ROYALBOT`", buttons=buttons)
            else:
                reply_pop_up_alert = "Please get your own Userbot,for more info visit @PATRICIA_SUPPORT!"
                await event.answer(reply_pop_up_alert, cache_time=0, alert=True)

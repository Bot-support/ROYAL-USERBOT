...
from ROYAL import CMD_LIST, LOAD_PLUG, LOGS, SUDO_LIST, bot
from ROYAL.helper.exceptions import CancelProcess

ENV = bool(os.environ.get("ENV", False))
if ENV:
    from ROYAL.ROYALConfig import Config
else:
...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"ROYAL/plugins/{shortname}.py")
        name = "ROYAL.plugins.{}".format(shortname)
        ...
        # support for uniborg
        sys.modules["uniborg.util"] = ROYAL.utils
        mod.Config = Config
        mod.borg = bot
        mod.edit_or_reply = edit_or_reply
        # support for paperplaneextended
        sys.modules["ROYAL.events"] = ROYAL.utils
        spec.loader.exec_module(mod)
        # for imports
...
def remove_plugin(shortname):
    try:
        try:
            ...
            name = f"ROYAL.plugins.{shortname}"
...
def errors_handler(func):
    async def wrapper(errors):
        try:
            ...
            link = "[here](https://t.me/Blaze_Oop)"
            ...
            ftext += "--------BEGIN ROYALBOT TRACEBACK LOG--------"
            ftext += "\nDate: " + date
...
def start_assistant(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"ROYAL/plugins/assistant/{shortname}.py")
        name = "ROYAL.plugins.assistant.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        ...
        mod.tgbot = bot.tgbot
        spec.loader.exec_module(mod)
        sys.modules["ROYAL.plugins.assistant" + shortname] = mod
